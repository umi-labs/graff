...
"#,
    ...
    let result = std::process::Command::new("cargo")
        .args([
            ...
            "render",
            "--spec",
            test_dir.path().join("test_spec.yaml").to_str().unwrap(),
            "--out",
...
fn test_cli_error_handling_missing_columns() {
    ...
    let result = std::process::Command::new("cargo")
        .args([
            ...
            "nonexistent_column",
            "--out",
            test_dir.path().join("output.png").to_str().unwrap(),
        ])
        .output();
    ...
    assert!(
        !output.status.success(),
        "Command should fail for missing column"
    );
...
fn test_cli_help_output() {
    ...
    assert!(
        ...
        "Help should mention render command"
    );
}
...
fn test_cli_with_theme_option() {
    ...
    assert!(
        output.status.success(),
        "CLI command with theme should succeed"
    );
    assert!(output_path.exists(), "Output file was not created");
}
...
fn test_cli_with_scale_option() {
    ...
    assert!(
        output.status.success(),
        "CLI command with scale should succeed"
    );
    assert!(output_path.exists(), "Output file was not created");
}
...
fn test_cli_with_format_option() {
    ...
    assert!(
        output.status.success(),
        "CLI command with PNG format should succeed"
    );
