    ...
    DB::ErrorType: 'static + std::error::Error + Send + Sync,
{
    ...
    for i in 0..df.height().min(1000) { // Limit points for performance but allow more than other charts
    ...
    let x_min = data_points.iter().map(|(x, _)| *x).fold(f32::INFINITY, f32::min);
    let x_max = data_points.iter().map(|(x, _)| *x).fold(f32::NEG_INFINITY, f32::max);
    let y_min = data_points.iter().map(|(_, y)| *y).fold(f32::INFINITY, f32::min);
    let y_max = data_points.iter().map(|(_, y)| *y).fold(f32::NEG_INFINITY, f32::max);
    ...
    chart
        ...
        .label(&format!("{} vs {}", config.y.as_ref().unwrap(), config.x.as_ref().unwrap()))
        .legend(|(x, y)| Circle::new((x + 5, y), style.layout.elements.line_points, point_color.filled()));
